...
public class LibreRawGlucoseOOPData: NSObject, Decodable, LibreRawGlucoseWeb {
    ...
    func glucoseData() -> (libreRawGlucoseData:[LibreRawGlucoseData], sensorState:LibreSensorState, sensorTimeInMinutes:Int?) {
        ...
        // check the order, first should be the smallest id, id is sensor time in minutes, means first should be the oldest
        // if not, reverse it
        ...
        for realTimeGlucose in history {
            ...
            let libreRawGlucoseData = LibreRawGlucoseData(timeStamp: creationTimeStamp.addingTimeInterval(-60 * Double(sensorTimeInMinutes - realTimeGlucose.id!)), glucoseLevelRaw: realTimeGlucose.value!)
    ...
    var valueError: Bool {
        ...
        if let realTimeGlucose = realTimeGlucose, let value = realTimeGlucose.dataQuality {
            return value != 0
        }
        return false
    }
